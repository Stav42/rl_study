...
class VANILLA_GRADIENT:
    ...
    def update(self):
        ...
        deltas = torch.tensor(running_reward) - torch.tensor(self.values)
        ...
        val_loss = mse_loss(torch.tensor(self.values, requires_grad = True, dtype=torch.float32), torch.tensor(self.returns_val,  requires_grad = True, dtype=torch.float32))
        val_loss.backward()
